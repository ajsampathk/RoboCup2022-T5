...
class robot():
    ...
    def move(self,x,y):
        ...
        if not (abs(self.current_state.pose.position.y-y) <0.05 and abs(self.current_state.pose.position.x-x) <0.05 ):
            ...
            updated_state.twist.linear.y =(0.5*(abs(dy)/(abs(dx)+abs(dy)))*cmp(y-self.current_state.pose.position.y,0))
            updated_state.twist.linear.x =(0.5*(abs(dx)/(abs(dx)+abs(dy)))*cmp(x-self.current_state.pose.position.x,0))
